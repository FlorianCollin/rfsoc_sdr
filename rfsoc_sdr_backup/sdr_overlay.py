...
from data import qam_gen, sin_gen, plot_fft, plot_scatter, LPF

# 5 DACs 1 ADC
class SdrOverlay(Overlay):

    def __init__(self, bitstream_path, Fs_dac_tile_0 = 6144, Fs_dac_tile_1 = 6144, Fs_adc_tile_0 = 4096):
        """__init__ method of SdrOverlay class

        Args:
            bitstream_path (str): The relative path of the bitstream file
            Fs (int, optional): sampling rate of tile 0 and tile 1. Defaults to 6144 (Maximum).
        """
        ...
        xrfclk.set_ref_clks()
        self.ref_clock = 409.6


        # For easy Class modification
        self.dacs_tiles = [0, 1]
...
#             raise ValueError("Fs must be a multiple of 409.6 MHz")
        ...
        # Mixer Settings DEFAULT MODE = "IQ"
        ### DACs ###
        # TILE 0 DAC
        ...
        # In reality bram counter and uram counter are different
        # For uram counter you need to set 1 more register : step
        # For ddr4 this is yet another control block to manage we will call them all controller_xx
        # class methods will allow this abstraction
        ...
        print("By default all DAC and ADC are set in IQ mode!")

########### Driver and Capture ##################

    def load_data(self, tile, dac, data):
        """ 
        Write to the memory space associated with the driver of a DAC.
        ...
        data : 1-D ndarray (in32) (IQ or Real)
        """
        self.check_if_dac_is_valid(tile, dac)
    ...
    def adc_capture(self, tile = 0, adc = 0, number_of_32bits_samples = 1024):
        """ Start the capture of the adc output into BRAM """
        self.check_if_adc_is_valid(tile, adc)
    ...
    def get_data(self, tile = 0, adc = 0, number_of_32_bits_samples = 1024, mode="IQ"):
    ...
    def set_dac_controller(self, tile, dac, number_of_32bits_samples):
        """ Set the controller associated with the driver of a DAC. """
        self.check_if_dac_is_valid(tile, dac)
...
########## DC Setting ###################
    ...
    def set_dac_tile_pll(self, tile, Fs = 6144):
        """set the PLL Config of a dacs tile

        Args:
            tile (int): tile number (0 or 1)
            Fs (int, optional): Sampling Rate in MHz (MSPs), Fs must be a multiple of 409.6 MHz. Defaults to 6144.
        """
        if tile not in [0, 1]:
            raise ValueError("tile must be in [0, 1]")
        self.rfdc.dac_tiles[tile].DynamicPLLConfig(1, self.ref_clock, Fs)
    
    def set_adc_tile_pll(self, tile = 0, Fs = 4096):
        """set the PLL Config of an adcs tile

        Args:
            tile (int, optional): tile number only 0 accepted. Defaults to 0.
            Fs (int, optional):  Sampling Rate in MHz (MSPs), Fs must be a multiple of 409.6 MHz. Defaults to 4096.
        """
        if tile not in [0]:
            raise ValueError("tile must be in [0]")
    ...
    def set_all_pll(self, Fs = 4096):
        ...
        for tile in self.adcs_tiles:
            self.set_adc_tile_pll(tile = tile, Fs = Fs)

    def set_dac_nco(self, tile, dac, nco):
        self.check_if_dac_is_valid(tile, dac)
        self.rfdc.dac_tiles[tile].blocks[dac].MixerSettings['Freq'] = nco
    
    def set_adc_nco(self, tile, adc, nco):
        self.check_if_adc_is_valid(tile, adc)
        self.rfdc.adc_tiles[tile].blocks[adc].MixerSettings['Freq'] = nco

    def set_dac_tile_nco(self, tile, nco_list):
        """ Set all the DAC NCO (MHz) parameters in a tile with the associated NCOs in the nco_list. """
        if tile not in [0, 1]:
            raise ValueError("tile must be in [0,1]")
    ...
    def set_adc_decmation(self, tile = 0, decimation_factor = 2):
        """Set the decimation factor of a DAC tile. 
        WARNING : tile must be set in IQ mode

        Args:
            tile (int, optional): tile number, must be 0 for this Overlay. Defaults to 0.
        ...
        self.rfdc.adc_tiles[tile].SetupFIFO(False)
        if (decimation_factor == 2):
            self.rfdc.adc_tiles[tile].FabClkOutDiv = 2
        elif (decimation_factor == 4):
            self.rfdc.adc_tiles[tile].FabClkOutDiv = 3
        else:  # 8
            self.rfdc.adc_tiles[tile].FabClkOutDiv = 4

        self.rfdc.adc_tiles[tile].InterpolationFactor = decimation_factor
        if (tile == 0):
            self.rfdc.adc_tiles[tile].blocks[0].InterpolationFactor = decimation_factor
            self.rfdc.adc_tiles[tile].blocks[0].IntrClr = 4294967295
        else:
            raise ValueError("tile must be 0")
    ...
    def check_if_dac_is_valid(self, tile, dac):
        if tile not in [0, 1]:
            raise ValueError("Tile must be in [0,1]")
        ...
        if (tile == 0 and dac not in self.dacs_tile0) or (tile == 1 and dac not in self.dacs_tile1):
            raise ValueError(
                "The dac is not enable, please check the info method to see the available resources.")
        return 0
            
    def check_if_adc_is_valid(self, tile, adc):
        # Only one adc valid
        if (tile == 0) and (adc == 0):
            return 0
        else:
    ...
    def check_bram_width(self, number_of_32bits_samples):
        factor = int(1024/32)
    ...
    def check_ddr4_width(self, number_of_32bits_samples):
        factor = int(512/32)
        if number_of_32bits_samples % factor != 0:
    ...
    def check_data_width(self, tile, dac, number_of_32bits_samples):
        ...
        if (tile == 0):
            if (dac == 0):
               self.check_bram_width(number_of_32bits_samples)
            elif (dac == 1):
                self.check_bram_width(number_of_32bits_samples)
            elif (dac == 2):
                self.check_bram_width(number_of_32bits_samples)
            ...
            if (dac == 2):
                self.check_ddr4_width(number_of_32bits_samples)
            elif (dac == 3):
                self.check_bram_width(number_of_32bits_samples)
            else:
                raise ValueError("dac value is imposible")
    ...
    def set_dac_tile_real(self, tile = 0):
        """ Set a tile in Real mode :
            interpolation factor = 1 (for real it was 1 only)
            number of 16 bits samples (axi stream) = 16 (constant)
            FabClkOutDiv = 2 (because of interpolation factor)
        """
        if tile not in [0, 1]:
            raise ValueError("tile must be in [0, 1]")
    ...
    def set_dac_tile_IQ(self, tile = 0, interpolation_factor = 2, nco_freq = -1500, nz = 2):
        """ Set all the DAC into a tile to a specific configuraiton with IQ->Real mode

        Args:
            tile (int, optional): the DAC tile number. Defaults to 0.
            interpolation_factor (int, optional): interpolation factor (2, 4, or 8). Defaults to 2.
            nco_freq (int, optional): NCO frequency in MHz. Defaults to -1500.
    ...
    def set_adc_tile_IQ(self, tile = 0, decimation_factor = 2, nco_freq = 1500, nz = 2):
        """ Set the ADC tile 0 in IQ mode

        Args:
            decimation_factor (int, optional): ADC decimation factor (2, 4 or 8). Defaults to 2.
        """
        if tile != 0:
            raise ValueError ("tile must be 0 for this overlay !")
        
        self.adcs_tiles_mode[tile] = "IQ"
        self.rfdc.adc_tiles[tile].blocks[0].MixerSettings = {
                'CoarseMixFreq':  0,
                'EventSource':    2,
                'FineMixerScale': 0,
                'Freq':           nco_freq,
        ...
        self.rfdc.adc_tiles[tile].blocks[0].NyquistZone = nz
        self.rfdc.adc_tiles[tile].blocks[0].UpdateEvent(xrfdc.EVENT_MIXER)
    ...
    def demo_sin(self, Fs = 4096e6, f = 128e6, capture_size = None):
        ...
        if (Fs > 4096e6):
            ...
            for tile in self.adcs_tiles:
                self.set_adc_tile_pll(tile, Fs = 4096)
        else:
    ...
    def info(self):
        ...
        print("\tTILE 1")
        # print("\t\tDAC 10 : BRAM(2) 1M")
        # print("\t\tDAC 11 : BRAM(3) 512K")
        print("\t\tDAC 12 : DDR4(0) 4G")
        print("\t\tDAC 13 : URAM(0) 2M")
        print("\tADCs Info :")
        print("\tTILE 0: ")
        print("\t\tADC 00 : BRAM(0) 1M")
        # print("One 32 Bits samples is equal to one IQ (Complex) samples (not true in Real)")
        # print("Scale ref : 1M = 250e3 32bits samples")
        self.print_config()

    def print_config(self):
        """Print the PLL Config fot tile0 and tile1 and all DACs Mixer Settings
        """
        print("\n\nConfiguration :\n")
        print("DACs :")
